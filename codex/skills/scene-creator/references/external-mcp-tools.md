...
External 场景包制作模式固定暴露 12 个工具。只使用线上工具 Schema 中实际存在的 action；下表只说明用途和顺序，不能替代 `tools/list`。
...
| `get_diagnosis_doc` | 读取服务端共享契约、示例、Battle、Verify、配置和诊断知识 | External 调用必须传 `task_id`；审计只保存 topic 和返回文档名，不保存正文；MCP Knowledge 是共享来源 |
...
| `workflow_tpe_manage` | Preview、创建、更新、挂载、发布和 bubble Workflow TPE | 创建必须传 `scene_package_id` 和根对象 `output_schema`；bubble 首次返回 `run_id`，后续轮询同一运行 |
...
- Scene Preview：校验脚本 AST、声明工具、input/output/文件契约和确定性字面规则。
- Hub 保存：校验场景包 orchestration 结构和映射。
- 供应商 `test_tool`：获取直连 MCP 真实返回样本，并可校验候选 Schema。
- Bubble Runtime：FA 使用 Schema 桩值，普通/文件工具真实执行，仅 `shell` 按需创建沙箱。
- Max Runtime：执行真实 FA、默认工具、文件和完整 C2 业务行为。
