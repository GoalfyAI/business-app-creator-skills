...
test -f "$HOME/.codex/.env" && grep '^BUSINESS_APP_CREATOR_QA_API_KEY=' "$HOME/.codex/.env"   # 密钥配了吗？
```
...
检查：`test -f "$HOME/.codex/.env" && grep '^BUSINESS_APP_CREATOR_QA_API_KEY=' "$HOME/.codex/.env"` —— 已存在则跳到第 3 步。
...
拿到密钥后写入 `~/.codex/.env` 的 `BUSINESS_APP_CREATOR_QA_API_KEY` 行：
...
- 文件不存在时创建它
- 写入后重跑一次检查，确认找到 `BUSINESS_APP_CREATOR_QA_API_KEY` 再继续
- 全程不回显密钥内容
...
| 现象 | 处理 |
|---|---|
| `401 Unauthorized` | 确认 `~/.codex/.env` 里有 `BUSINESS_APP_CREATOR_QA_API_KEY`，且该密钥在 GoalfyMax 未被撤销，然后请用户再彻底重启一次。只检查变量是否存在，不要输出其值 |
...
2. 更新 `~/.codex/.env` 的 `BUSINESS_APP_CREATOR_QA_API_KEY` 行，保留其他环境变量，
