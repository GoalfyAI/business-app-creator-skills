...
> **1.21.3 起密钥变量改名**：`SCENE_CREATOR_API_KEY` → `BUSINESS_APP_CREATOR_QA_API_KEY`。已装用户升级后要把 `~/.codex/.env` 里那一行的键名改成新名（值不变），否则 MCP 401。
...
2. Agent 只替换 `~/.codex/.env` 中的 `BUSINESS_APP_CREATOR_QA_API_KEY` 行，保留其他环境变量；不得要求
