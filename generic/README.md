...
```json
{
  "mcpServers": {
    "business-app-creator": {
      ...
      "headers": {
        "Authorization": "Bearer ${BUSINESS_APP_CREATOR_QA_API_KEY}"
      }
    }
  }
}
...
把密钥放进环境变量 `BUSINESS_APP_CREATOR_QA_API_KEY`。客户端不支持环境变量占位符时，直接把
`${BUSINESS_APP_CREATOR_QA_API_KEY}` 替换为密钥本身，但**不要**把替换后的文件提交到任何仓库。
