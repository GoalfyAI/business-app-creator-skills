...
把完整的个人 API 密钥提供给当前 Agent，由 Agent 在保留原有环境变量的前提下，把它配置到
`~/.codex/.env` 的 `SCENE_CREATOR_API_KEY`，并把文件权限设为仅本人可读。用户不需要自行编辑
配置文件。Agent 不得回显密钥、在命令输出或 diff 中暴露密钥，也不得把密钥写入仓库、Skill、
`AGENTS.md`、检查点或日志。配置完成后彻底退出并重新打开 Codex。
