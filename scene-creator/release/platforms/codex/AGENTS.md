# Scene Creator Install & Update Guide — Codex
...
```bash
export PACKAGE_ROOT="/absolute/path/to/scene-creator-codex"
...
**Write the exact QA Personal API Key into `~/.codex/.env` as `SCENE_CREATOR_API_KEY=<key>`, then run `chmod 600 ~/.codex/.env`. Do not send the key to me.**
