# Scene Creator — Claude Code

This package installs the audited Scene Creator MCP and the
`scene-creator` Skill together. It targets US QA:
...
`SCENE_CREATOR_API_KEY` is only the local variable name; the plugin sends its
value as `Authorization: Bearer <GoalfyMax Personal API Key>`:

```json
{
  "env": {
    "SCENE_CREATOR_API_KEY": "<your exact Personal API Key>"
  }
}
```
...
1. Confirm the `scene-creator` plugin and Skill are loaded.
2. Run `/mcp` and confirm `scene-creator` is connected.
