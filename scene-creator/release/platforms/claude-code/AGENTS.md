# Scene Creator Install & Update Guide — Claude Code

Follow this runbook exactly. It installs one local marketplace plugin containing
the External MCP configuration and the `scene-creator` Skill.
...
```bash
export PACKAGE_ROOT="/absolute/path/to/scene-creator-claude-code"
...
**Merge `"SCENE_CREATOR_API_KEY": "<exact key>"` into the existing `env` object in `~/.claude/settings.json`. Preserve all other settings and do not send the key to me.**
...
1. `scene-creator` is installed and the Skill can be selected.
2. `/mcp` shows `scene-creator` connected.
