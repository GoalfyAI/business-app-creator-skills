...
SPEC = importlib.util.spec_from_file_location("workflow_skill_release", SCRIPT_PATH)
...
def _copy_skill(tmp_path: Path) -> Path:
    copied = tmp_path / "workflow-authoring"
...
def test_checked_in_workflow_skill_release_is_current():
    manifest = release_module.check_release(SKILL_ROOT)

    assert manifest["skill_name"] == "workflow-authoring"
...
def test_platform_packages_share_one_source_and_exclude_release_files(tmp_path: Path):
    ...
    assert set(archives) == {
        f"workflow-authoring-codex-{version}.zip",
        f"workflow-authoring-claude-code-{version}.zip",
        f"workflow-authoring-generic-{version}.zip",
    }
    ...
    codex = contents[f"workflow-authoring-codex-{version}.zip"]
    claude = contents[f"workflow-authoring-claude-code-{version}.zip"]
    generic = contents[f"workflow-authoring-generic-{version}.zip"]
    codex_plugin = "workflow-authoring-codex/plugins/workflow-authoring"
    claude_plugin = "workflow-authoring-claude-code/plugins/workflow-authoring"
    assert f"{codex_plugin}/skills/workflow-authoring/agents/openai.yaml" in codex
    assert f"{claude_plugin}/skills/workflow-authoring/agents/openai.yaml" not in claude
    assert "workflow-authoring/agents/openai.yaml" not in generic
    assert "workflow-authoring-codex/.agents/plugins/marketplace.json" in codex
    assert "workflow-authoring-claude-code/.claude-plugin/marketplace.json" in claude
    ...
    for relative_path in common_files:
        generic_member = f"workflow-authoring/{relative_path}"
        assert (
            codex[f"{codex_plugin}/skills/workflow-authoring/{relative_path}"]
            == generic[generic_member]
        )
        assert (
            claude[f"{claude_plugin}/skills/workflow-authoring/{relative_path}"]
            == generic[generic_member]
        )

    manifest = _manifest()
    assert set(generic) == {
        f"workflow-authoring/{relative_path}" for relative_path in manifest["source_files"]
...
def test_platform_mcp_templates_use_env_key_and_live_external_contract():
    for platform in ("codex", "claude-code"):
        ...
        assert "GOALFY_WORKFLOW_API_KEY" in mcp_text
        assert "12" in docs
