---
name: app_creator
description: 当开发者需要为一个已上线的 GoalfyMax 场景包制作业务系统（前后端部署物：业务系统前端 + 专属后端 + 数据集模板 schema），或修订、诊断已发布的业务系统时使用。业务系统承担完整 SaaS 链路：看板展示信息、通过场景包生产和流转信息、用户自主编辑信息、信息回流数据库。制作场景包本身不用本 Skill（那是 scene-creator 的事）。
---
