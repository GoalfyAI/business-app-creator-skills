...
从零把场景包制作能力接进 Claude Code，约 5 分钟。

平台专属的完整说明见 [`claude-code/README.md`](../claude-code/README.md)，
Agent 可直接执行的安装流程见 [`claude-code/AGENTS.md`](../claude-code/AGENTS.md)。

> **想省事的话**：把 [claude-code/AGENTS.md](https://raw.githubusercontent.com/GoalfyAI/business-app-creator-skills/business-qa/claude-code/AGENTS.md) 直接发给你的 Agent，
...
把完整密钥交给当前 Agent，让它写入 `~/.claude/settings.json`：

```json
{
  "env": {
    "BUSINESS_APP_CREATOR_QA_API_KEY": "<你的个人 API 密钥>"
  }
}
```

Agent 会合并写入并保留其他配置，你不需要自己编辑文件。

## 第 4 步 — 重启

执行 `/reload-plugins`，或彻底退出 Claude Code 后重新打开。
