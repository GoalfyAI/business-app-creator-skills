...
QA 版读取的环境变量是 `BUSINESS_APP_CREATOR_QA_API_KEY`，和正式版的 `BUSINESS_APP_CREATOR_QA_API_KEY`
分开，两个密钥可以同时保留在本机。
...
卸载插件并移除市场后，按仓库根目录 README 重新安装正式版（不带 `#business-qa`），环境变量用 `BUSINESS_APP_CREATOR_QA_API_KEY`。
