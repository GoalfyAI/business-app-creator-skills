...
grep BUSINESS_APP_CREATOR_QA_API_KEY "$HOME/.claude/settings.json"     # 密钥配了吗？
```
...
检查：`grep BUSINESS_APP_CREATOR_QA_API_KEY "$HOME/.claude/settings.json"` —— 已存在则跳到第 3 步。
...
拿到密钥后，把它**合并写入** `~/.claude/settings.json` 的 `env.BUSINESS_APP_CREATOR_QA_API_KEY`：

```json
{
  "env": {
    "BUSINESS_APP_CREATOR_QA_API_KEY": "<用户提供的密钥>"
  }
}
```
...
- 文件不存在时创建所需结构
- 写入后重跑一次 grep 检查，确认找到 `BUSINESS_APP_CREATOR_QA_API_KEY` 再继续
- 全程不回显密钥内容
...
| 现象 | 处理 |
|---|---|
| `401 Unauthorized` | 确认 `~/.claude/settings.json` 里有 `BUSINESS_APP_CREATOR_QA_API_KEY`，且该密钥在 GoalfyMax 未被撤销，然后请用户再彻底重启一次。只检查变量是否存在，不要输出其值 |
...
2. 把新密钥**合并写入** `~/.claude/settings.json` 的 `env.BUSINESS_APP_CREATOR_QA_API_KEY`，
   覆盖旧值，保留其余全部配置
