---
name: app-creator
description: 当开发者需要为一个已上线的 GoalfyMax 场景包制作业务应用（前后端部署物：业务应用前端 + 专属后端 + 业务应用数据集模板），或修订、诊断已发布的业务应用时使用。业务应用承担完整 SaaS 链路：看板展示信息、通过场景包生产和流转信息、用户自主编辑信息、信息回流数据库。制作场景包本身不用本 Skill（那是 business-app-creator 的事）。
---
...
## 前置：本 Skill 与 business-app-creator 的分工

| | business-app-creator | app-creator（本 Skill） |
|---|---|---|
...
**先包后应用**：本 Skill 只对**已上线**的场景包开工。目标包未上线时，先用 business-app-creator 完成制作与上线。工单不强制分开：接着场景包工单继续干、或另开一张工单，由开发者定（business-app-creator 约束 8）。

**三个不能混的概念**（与 business-app-creator 前置条件同一口径）：① 有业务路线编排的场景包才能有业务应用——应用的每个入口都是一条路线；② 业务应用与场景包强绑定——一个 `business_ui` 恰好挂一个包，入口表单 / 中途表单 / 交付字段 / 事件全部来自它的契约，包换契约应用就要重新部署；③ 业务应用（编排形）是交付给用户的产品，场景包是资产载体，对开发者说前者、对工具说后者。
...
| 制作或修改场景包本身 | 不用本 Skill，转 business-app-creator |
