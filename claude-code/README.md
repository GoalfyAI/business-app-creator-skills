# 智能应用制作 — Claude Code 插件

把一段业务流程做成 GoalfyMax 上可以直接交给用户用的智能应用，并诊断、优化、迭代已有的智能应用。智能应用是主要产物，场景包是它下面的能力资产。
...
- 从业务访谈出发，设计智能应用：用户画像、旅途、页面、表单、数据面
- 制作智能应用下的能力资产：业务路线编排、Workflow、FastAgent、工具集、任务点（场景包）
- 制作智能应用数据集模板与配套前后端应用，预部署、自动化测试、上线
...
```bash
git clone -b business-qa https://github.com/GoalfyAI/business-app-creator-skills.git
...
`env.BUSINESS_APP_CREATOR_QA_API_KEY`，保留其他全部配置。你不需要自行编辑配置文件。
...
`BUSINESS_APP_CREATOR_QA_API_KEY` 只是本地变量名，插件会将它的值作为
