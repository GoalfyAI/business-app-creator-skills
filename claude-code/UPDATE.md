...
> **1.21.3 起密钥变量改名**：`SCENE_CREATOR_API_KEY` → `BUSINESS_APP_CREATOR_QA_API_KEY`。已装用户升级后要在 `~/.claude/settings.json` 的 `env` 里把键名改成新名（值不变），否则 MCP 401。
...
2. Agent 只替换 `~/.claude/settings.json` 中的 `env.BUSINESS_APP_CREATOR_QA_API_KEY`，保留其他所有配置；
